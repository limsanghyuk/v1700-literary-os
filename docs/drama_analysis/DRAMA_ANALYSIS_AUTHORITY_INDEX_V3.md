...
분산되어 있던 exact schema, 직접독해, 검증, 계보, 세션 안전, 작품 상태를 연결하고 새 대화창이 과거 대화 전체를 읽지 않고 즉시 분석을 실행하도록 권위와 최소 로드 세트를 고정한다.
...
1. `SCHEMA_CONTRACTS_V2.md`
   - exact keyset, type, enum, ID, FK, invariants.
2. `DRAMA_ANALYSIS_CURRENT_OPERATING_SUPPLEMENT_2026-07-15.md`
   - 현재 실행 단위, DB 삽입, EXT6 보류.
3. `DRAMA_NEW_CONVERSATION_EXECUTION_GUIDE_V1.md`
   - 신규 세션의 통합 실행 순서와 완료 체크리스트.
4. `DRAMA_ENSEMBLE_TRACKING_AND_EDGE_SELECTIVITY_POLICY_V1.md`
   - 앙상블 인물·관계 폭 채택, LocalEdge 과밀 차단, 후보 전수 처분.
5. `DRAMA_CLOSE_READING_MASTER_PROTOCOL_V3.md`
   - 직접독해·내용 깊이·Stage별 저작.
6. `DRAMA_VALIDATION_AND_RELEASE_GATES_V3.md`
   - 구조·내용·반게이밍·Stage04·패키지 검증.
7. `DRAMA_LINEAGE_PACKAGE_HANDOFF_V2.md`
   - SourceLock, QuarterAudit, quarantine, supersession, ZIP, 허브 편입.
8. `DRAMA_SESSION_EXECUTION_SAFETY_V1.md`
   - 세션 한도, 영속화, 중단 복구.
9. `EXT6_DEFERRED_SIDECAR_POLICY_V1.md`
   - EXT6 현재 보류.
10. `DRAMA_STAGE_EXT6_CONTRACT_MATRIX_V3.md`
    - EXT6 재활성화 시 보존 계약.
11. `DRAMA_ANALYSIS_DATABASE_STATUS_2026-07-15.json`
    - 현재 49작품, 43완료, 6잔여.
12. 최신 `docs/sessions/*drama*/README.md`
    - 작업 산출물·SHA·다음 진입점.
...
remaining upgrades: 6
canonical promoted works: 13
```
...
사용자 명시 승인에 따라 다음 13작품을 CANONICAL로 승격한다.
...
DATABASE_STATUS: INTEGRATED_VALIDATED_PROGRESS_43_OF_49
CANONICAL_PROMOTION: USER_APPROVED_13_WORKS
```
