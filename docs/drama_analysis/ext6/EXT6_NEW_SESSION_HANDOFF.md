...
- 새 세션에 현재 신뢰 baseline ZIP이 없으면 파일명·SHA·파일 수를 추정하거나 임의 생성하지 않는다.
- 검증된 직전 통합 DB와 그 이후 완료된 개별 EXT6 패키지가 모두 존재할 때만 append-only로 재구성한다.
- 복구 후 기존 baseline 파일의 경로·CRC·내용 해시 불일치가 0인지 확인한다.
- 개별 완료 패키지가 없거나 SHA를 확인할 수 없으면 통합은 `BASELINE_RECOVERY_HOLD`로 두고 대상 작품의 독립 분석·checkpoint까지만 진행한다.
- 허브의 완료 수치와 로컬 산출물이 충돌하면 로컬 실물 검증 결과를 우선하고 pointer/status를 정정한다.
- 복구 때문에 새 방법 버전이나 새 권위 문서를 만들지 않는다.
...
3. 없으면 제8절의 복구 규칙을 적용하고 상태를 명시한다.
4. pointer의 현재 작품 《그대웃어요》를 확인한다.
5. 기존 checkpoint가 있으면 마지막 완료 지점 다음부터 재개한다.
6. V1.2 전체 10개 계층이 끝나기 전 완료 처리하지 않는다.
