...
2. Run `python tools/run_stage138_losdb_storage_contracts.py`.
3. Run `python tools/run_stage138_release_gate.py`.
...
8. Commit on a dedicated `stage138-*` branch.
9. Push branch, open PR, and wait for GitHub Actions to pass.
10. Merge PR, tag `v1700-stage138`, and publish release assets.
11. Replace any generic workflow-generated assets with the official Stage138 handoff ZIP if needed.

## Official Asset Policy

- The release page must expose the Stage138 ZIP named in `package_manifest.json`.
- The SHA256 sidecar must match the ZIP.
- `release/current/stage138_release_asset_manifest.json` is the local source of truth for release asset alignment.
