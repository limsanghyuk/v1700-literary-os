...
TARGET_STAGE = "stage175"
TARGET_REPORT = "release/current/stage175_project_boundary_governor_report.json"
PACK_DIR = "release/current/stage175_project_boundary_governor_pack"
...
def run_stage175_project_boundary_governor(root: Path | None = None, mode: str = "active") -> dict[str, Any]:
    ...
    previous_gate = _load_json(root / "release/current/stage174_release_gate_report.json") or {}
    previous_report = _load_json(root / "release/current/stage174_release_policy_registry_report.json") or {}
    preflight = _load_json(root / "release/current/stage175_preflight_execution_report.json") or {}
    gitnexus = _load_json(root / "release/current/stage175_gitnexus_preflight_analysis_report.json") or {}
    comparison = _load_json(root / "release/current/stage175_package_comparison_report.json") or {}

    parts = _build_stage175_pack(root, previous_gate, previous_report, preflight, gitnexus, comparison)
    ...
    result: dict[str, Any] = {
        "stage": "175", "baseline_stage": "174", "title": "Project Boundary Governor",
        "status": "pass" if not issues else "blocked", "issues": issues,
        "page": "Page06 Governance Body", "mode": "PROJECT_BOUNDARY_GOVERNOR_LOCAL_ONLY",
        ...
        "stage176_lineage_review_gate_ready": not issues,
        "next_stage": "stage176", "next_stage_title": "Lineage Review Gate",
        **CORE_INVARIANTS,
        "default_authority_decision": "DENY", "deny_by_default": True,
        "branchpoint_lineage_preserved": not issues,
        "parts": {"previous_gate": _compact(previous_gate), **parts},
    }
    result["stage175_checksum"] = _stable_digest(result)
    _write_json(root / TARGET_REPORT, result)
    _write_json(root / "release/current/stage175_summary.json", _compact(result))
    return result


def _build_stage175_pack(root: Path, previous_gate: dict[str, Any], previous_report: dict[str, Any], preflight: dict[str, Any], gitnexus: dict[str, Any], comparison: dict[str, Any]) -> dict[str, dict[str, Any]]:
    stage = TARGET_STAGE
    if stage == "stage174":
...
def _blocked(issue: str) -> dict[str, Any]:
    return {"stage": "175", "baseline_stage": "174", "title": "Project Boundary Governor", "status": "blocked", "issues": [issue], "page": "Page06 Governance Body", **CORE_INVARIANTS, "branchpoint_lineage_preserved": False}
