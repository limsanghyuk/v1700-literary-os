...
TARGET_STAGE = "stage182"
TARGET_REPORT = "release/current/stage182_upgrade_simulation_compatibility_sandbox_report.json"
PACK_DIR = "release/current/stage182_upgrade_simulation_compatibility_sandbox_pack"
...
def run_stage182_upgrade_simulation_compatibility_sandbox(root: Path | None = None, mode: str = "active") -> dict[str, Any]:
    ...
    previous_gate = _load_json(root / "release/current/stage181_release_gate_report.json") or {}
    previous_report = _load_json(root / "release/current/stage181_migration_plan_compiler_report.json") or {}
    preflight = _load_json(root / "release/current/stage182_preflight_execution_report.json") or {}
    gitnexus = _load_json(root / "release/current/stage182_gitnexus_preflight_analysis_report.json") or {}
    comparison = _load_json(root / "release/current/stage182_package_comparison_report.json") or {}
    ...
    result: dict[str, Any] = {"stage":"182", "baseline_stage":"181", "title":"Upgrade Simulation and Compatibility Sandbox", "status":"pass" if not issues else "blocked", "issues":issues, "page":"Page07 Evolution Body", "mode":"UPGRADE_SIMULATION_AND_COMPATIBILITY_SANDBOX_LOCAL_ONLY", "previous_stage_gate_inherited":previous_gate.get("status")=="pass", "previous_stage_report_inherited":previous_report.get("status")=="pass", "preflight_execution_report_pass":preflight.get("status")=="pass", "gitnexus_preflight_analysis_pass":_gitnexus_ok(gitnexus), "package_comparison_report_pass":comparison.get("status")=="pass", "next_stage":"stage183", "next_stage_title":"Future Absorption and Deprecation Planner", **CORE_INVARIANTS, "default_evolution_decision":"DEFER", "branchpoint_lineage_preserved":not issues, "parts":{"previous_gate":_compact(previous_gate), **parts}}
    result["stage182_checksum"] = _stable_digest(result)
    _write_json(root / TARGET_REPORT, result)
    _write_json(root / "release/current/stage182_summary.json", _compact(result))
    return result
...
def _build_pack(root: Path, previous_gate: dict[str, Any], previous_report: dict[str, Any], preflight: dict[str, Any], gitnexus: dict[str, Any], comparison: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    sandbox = {"stage": stage, "title": "Upgrade Simulation Sandbox", "status": "pass", "simulation_runtime_side_effects": 0, "write_operation_count": 0, "provider_default_calls": 0, "issues": []}
    compat = {"stage": stage, "title": "Compatibility Simulation Matrix", "status": "pass", "simulations": ["manifest_compatibility", "release_gate_compatibility", "historical_test_lane", "package_integrity", "workflow_compatibility", "rollback_rehearsal"], "compatibility_blockers_recorded": True, "issues": []}
    rollback = {"stage": stage, "title": "Rollback Rehearsal Snapshot", "status": "pass", "rollback_rehearsal_pass": True, "live_state_mutated": False, "issues": []}
    entry = {"stage": stage, "title": "Stage183 Entry Criteria", "status": "pass", "stage183_absorption_planning_ready": True, "issues": []}
    return {"upgrade_simulation_sandbox": sandbox, "compatibility_simulation_matrix": compat, "rollback_rehearsal_snapshot": rollback, "stage183_entry_criteria": entry}


def _blocked(issue: str) -> dict[str, Any]: return {"stage":"182", "baseline_stage":"181", "title":"Upgrade Simulation and Compatibility Sandbox", "status":"blocked", "issues":[issue], "page":"Page07 Evolution Body", **CORE_INVARIANTS, "branchpoint_lineage_preserved":False}
