...
TARGET_STAGE = "stage176"
TARGET_REPORT = "release/current/stage176_lineage_review_gate_report.json"
PACK_DIR = "release/current/stage176_lineage_review_gate_pack"
...
def run_stage176_lineage_review_gate(root: Path | None = None, mode: str = "active") -> dict[str, Any]:
    ...
    previous_gate = _load_json(root / "release/current/stage175_release_gate_report.json") or {}
    previous_report = _load_json(root / "release/current/stage175_project_boundary_governor_report.json") or {}
    preflight = _load_json(root / "release/current/stage176_preflight_execution_report.json") or {}
    gitnexus = _load_json(root / "release/current/stage176_gitnexus_preflight_analysis_report.json") or {}
    comparison = _load_json(root / "release/current/stage176_package_comparison_report.json") or {}

    parts = _build_stage176_pack(root, previous_gate, previous_report, preflight, gitnexus, comparison)
    ...
    result: dict[str, Any] = {
        "stage": "176", "baseline_stage": "175", "title": "Lineage Review Gate",
        "status": "pass" if not issues else "blocked", "issues": issues,
        "page": "Page06 Governance Body", "mode": "LINEAGE_REVIEW_GATE_LOCAL_ONLY",
        ...
        "stage177_operational_safety_and_rollback_governance_ready": not issues,
        "next_stage": "stage177", "next_stage_title": "Operational Safety and Rollback Governance",
        **CORE_INVARIANTS,
        "default_authority_decision": "DENY", "deny_by_default": True,
        "branchpoint_lineage_preserved": not issues,
        "parts": {"previous_gate": _compact(previous_gate), **parts},
    }
    result["stage176_checksum"] = _stable_digest(result)
    _write_json(root / TARGET_REPORT, result)
    _write_json(root / "release/current/stage176_summary.json", _compact(result))
    return result


def _build_stage176_pack(root: Path, previous_gate: dict[str, Any], previous_report: dict[str, Any], preflight: dict[str, Any], gitnexus: dict[str, Any], comparison: dict[str, Any]) -> dict[str, dict[str, Any]]:
    stage = TARGET_STAGE
    if stage == "stage174":
...
def _blocked(issue: str) -> dict[str, Any]:
    return {"stage": "176", "baseline_stage": "175", "title": "Lineage Review Gate", "status": "blocked", "issues": [issue], "page": "Page06 Governance Body", **CORE_INVARIANTS, "branchpoint_lineage_preserved": False}
