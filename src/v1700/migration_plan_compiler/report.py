...
TARGET_STAGE = "stage181"
TARGET_REPORT = "release/current/stage181_migration_plan_compiler_report.json"
PACK_DIR = "release/current/stage181_migration_plan_compiler_pack"
...
def run_stage181_migration_plan_compiler(root: Path | None = None, mode: str = "active") -> dict[str, Any]:
    ...
    previous_gate = _load_json(root / "release/current/stage180_release_gate_report.json") or {}
    previous_report = _load_json(root / "release/current/stage180_architecture_drift_self_audit_report.json") or {}
    preflight = _load_json(root / "release/current/stage181_preflight_execution_report.json") or {}
    gitnexus = _load_json(root / "release/current/stage181_gitnexus_preflight_analysis_report.json") or {}
    comparison = _load_json(root / "release/current/stage181_package_comparison_report.json") or {}
    ...
    result: dict[str, Any] = {"stage":"181", "baseline_stage":"180", "title":"Migration Plan Compiler", "status":"pass" if not issues else "blocked", "issues":issues, "page":"Page07 Evolution Body", "mode":"MIGRATION_PLAN_COMPILER_LOCAL_ONLY", "previous_stage_gate_inherited":previous_gate.get("status")=="pass", "previous_stage_report_inherited":previous_report.get("status")=="pass", "preflight_execution_report_pass":preflight.get("status")=="pass", "gitnexus_preflight_analysis_pass":_gitnexus_ok(gitnexus), "package_comparison_report_pass":comparison.get("status")=="pass", "next_stage":"stage182", "next_stage_title":"Upgrade Simulation and Compatibility Sandbox", **CORE_INVARIANTS, "default_evolution_decision":"DEFER", "branchpoint_lineage_preserved":not issues, "parts":{"previous_gate":_compact(previous_gate), **parts}}
    result["stage181_checksum"] = _stable_digest(result)
    _write_json(root / TARGET_REPORT, result)
    _write_json(root / "release/current/stage181_summary.json", _compact(result))
    return result
...
def _build_pack(root: Path, previous_gate: dict[str, Any], previous_report: dict[str, Any], preflight: dict[str, Any], gitnexus: dict[str, Any], comparison: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    plan = {"stage": stage, "title": "Migration Plan Registry", "status": "pass", "migration_is_plan_only": True, "automatic_mutation_enabled": False, "automatic_release_promotion_enabled": False, "issues": []}
    graph = {"stage": stage, "title": "Migration Dependency Graph", "status": "pass", "graph_deterministic": True, "rollback_edges_present": True, "issues": []}
    risk = {"stage": stage, "title": "Migration Risk Scorecard", "status": "pass", "risk_dimensions": ["lineage", "manifest", "release_gate", "package", "rollback"], "sensitive_change_policy_refs_present": True, "issues": []}
    entry = {"stage": stage, "title": "Stage182 Entry Criteria", "status": "pass", "stage182_upgrade_simulation_ready": True, "issues": []}
    return {"migration_plan_registry": plan, "migration_dependency_graph": graph, "migration_risk_scorecard": risk, "stage182_entry_criteria": entry}


def _blocked(issue: str) -> dict[str, Any]: return {"stage":"181", "baseline_stage":"180", "title":"Migration Plan Compiler", "status":"blocked", "issues":[issue], "page":"Page07 Evolution Body", **CORE_INVARIANTS, "branchpoint_lineage_preserved":False}
