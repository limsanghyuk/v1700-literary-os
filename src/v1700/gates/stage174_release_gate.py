...
def run_stage174_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]

    if _active_version(root) != "stage174":
        existing = root / "release/current/stage174_release_gate_report.json"
        if existing.exists():
            try:
                return json.loads(existing.read_text(encoding="utf-8"))
            except Exception:
                pass
    stage = run_stage174_release_policy_registry(root)
    checks = {
        "previous_stage_gate_pass": _check(_report_pass(root, "release/current/stage173_release_gate_report.json")),
        "stage174_release_policy_registry_pass": _check(stage.get("status") == "pass"),
        "preflight_execution_report_pass": _check(_report_pass(root, "release/current/stage174_preflight_execution_report.json")),
        "gitnexus_preflight_analysis_pass": _check(_gitnexus_analysis_ok(root)),
        "package_comparison_report_pass": _check(_report_pass(root, "release/current/stage174_package_comparison_report.json")),
    ...
    result = {"stage": "174", "baseline_stage": "173", "title": "Release Policy and Registry", "status": "pass" if not issues else "blocked", "issues": issues, "checks": checks, "default_authority_decision": "DENY", **CORE_INVARIANTS, "branchpoint_lineage_preserved": not issues}
    out = root / "release/current/stage174_release_gate_report.json"
...
def _gitnexus_analysis_ok(root: Path) -> bool:
    path = root / "release/current/stage174_gitnexus_preflight_analysis_report.json"
    if not path.exists(): return False
    try: data=json.loads(path.read_text(encoding="utf-8"))
    except Exception: return False
...
def _docs_manifest_ok(root: Path) -> bool:
    expected=["docs/stages/stage174.md", "docs/proposals/stage174_release_policy_registry_proposal.md", "docs/architecture/stage174_release_policy_registry_blueprint.md", "docs/development/stage174_developer_handoff.md", "manifests/stage174_manifest.json", "manifests/stage174_release_policy_registry_manifest.json", "manifests/stage174_branchpoint_trace_manifest.json", "manifests/live_core_stage174_overlay.json", "release/current/stage174_release_asset_manifest.json", "release/current/stage174_release_policy_registry_report.json", "release/current/stage174_preflight_execution_report.json", "release/current/stage174_gitnexus_preflight_analysis_report.json", "release/current/stage174_package_comparison_report.json"]
...
def _procedure_alignment_ok(root: Path) -> bool:
    ...
    return all(token in text for token in ["stage174", "run_stage174_release_policy_registry.py", "run_stage174_release_gate.py"])
