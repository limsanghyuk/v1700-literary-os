...
def run_stage128_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]
    key = str(root.resolve())
    if key in _CACHE:
        return _CACHE[key]
    baseline = run_stage127_release_gate(root)
    stage = run_stage128(root)
    parts = stage.get("parts", {})
    checks = {
        "stage127_baseline_gate_pass": _check(baseline.get("status") == "pass"),
        "shared_character_read_only_adapter_pass": _check(parts.get("shared_character_adapter", {}).get("status") == "PASS"),
        "shared_world_read_only_adapter_pass": _check(parts.get("shared_world_adapter", {}).get("status") == "PASS"),
        "license_boundary_adapter_pass": _check(parts.get("license_boundary_adapter", {}).get("status") == "pass"),
        "project_isolation_guard_pass": _check(parts.get("project_isolation_guard", {}).get("status") == "pass"),
        "canon_conflict_report_pass": _check(parts.get("canon_conflict_report", {}).get("status") == "pass"),
        "read_only_absorption_enforced": _check(stage.get("read_only_absorption") is True and stage.get("cross_project_write") == 0),
        "no_shared_db_write_enabled": _check(stage.get("shared_world_source_of_truth_promotion_allowed") is False),
        ...
        "stage129_canon_governor_deferred": _check(stage.get("stage129_canon_governor_required") is True),
        "gitnexus_python_fallback_preflight_pass": _check(parts.get("gitnexus_preflight", {}).get("python_fallback", {}).get("status") == "PASS"),
    ...
    result = {
        "stage": "128",
        "baseline_stage": "127",
        "title": "SharedWorld / SharedCharacter Read-Only Absorption Release Gate",
        "status": "pass" if not issues and stage.get("status") == "pass" else "blocked",
        "issues": issues,
        "checks": checks,
        "stage128": _compact(stage),
    ...
    out = root / "release/current/stage128_release_gate_report.json"
...
def _compact(stage: dict[str, Any]) -> dict[str, Any]:
    keep = (
        "status", "stage", "baseline_stage", "title", "issues", "read_only_absorption",
        "unauthorized_cross_reads", "unauthorized_cross_writes", "cross_project_write",
        ...
        "python_fallback_used", "stage129_canon_governor_required",
    )
...
def _repo_doctor_ready(root: Path) -> bool:
    ...
    return "stage128" in text and "stage128_release_gate" in text and "stage128_read_only_absorption" in text
...
def _clean_packaging_status(root: Path) -> str:
    ...
    canonical = manifest.get("canonical_package") or "V1700_stage128_sharedworld_sharedcharacter_read_only_absorption_integrated_repository.zip"
    override = os.environ.get("V1700_STAGE128_PACKAGE")
