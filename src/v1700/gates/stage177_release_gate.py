...
def run_stage177_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]

    if _active_version(root) != "stage177":
        existing = root / "release/current/stage177_release_gate_report.json"
        if existing.exists():
            try:
                return json.loads(existing.read_text(encoding="utf-8"))
            except Exception:
                pass
    stage = run_stage177_operational_safety_rollback_governance(root)
    checks = {
        "previous_stage_gate_pass": _check(_report_pass(root, "release/current/stage176_release_gate_report.json")),
        "stage177_operational_safety_rollback_governance_pass": _check(stage.get("status") == "pass"),
        "preflight_execution_report_pass": _check(_report_pass(root, "release/current/stage177_preflight_execution_report.json")),
        "gitnexus_preflight_analysis_pass": _check(_gitnexus_analysis_ok(root)),
        "package_comparison_report_pass": _check(_report_pass(root, "release/current/stage177_package_comparison_report.json")),
    ...
    result = {"stage": "177", "baseline_stage": "176", "title": "Operational Safety and Rollback Governance", "status": "pass" if not issues else "blocked", "issues": issues, "checks": checks, "default_authority_decision": "DENY", **CORE_INVARIANTS, "branchpoint_lineage_preserved": not issues}
    out = root / "release/current/stage177_release_gate_report.json"
...
def _gitnexus_analysis_ok(root: Path) -> bool:
    path = root / "release/current/stage177_gitnexus_preflight_analysis_report.json"
    if not path.exists(): return False
    try: data=json.loads(path.read_text(encoding="utf-8"))
    except Exception: return False
...
def _docs_manifest_ok(root: Path) -> bool:
    expected=["docs/stages/stage177.md", "docs/proposals/stage177_operational_safety_rollback_governance_proposal.md", "docs/architecture/stage177_operational_safety_rollback_governance_blueprint.md", "docs/development/stage177_developer_handoff.md", "manifests/stage177_manifest.json", "manifests/stage177_operational_safety_rollback_governance_manifest.json", "manifests/stage177_branchpoint_trace_manifest.json", "manifests/live_core_stage177_overlay.json", "release/current/stage177_release_asset_manifest.json", "release/current/stage177_operational_safety_rollback_governance_report.json", "release/current/stage177_preflight_execution_report.json", "release/current/stage177_gitnexus_preflight_analysis_report.json", "release/current/stage177_package_comparison_report.json"]
...
def _procedure_alignment_ok(root: Path) -> bool:
    ...
    return all(token in text for token in ["stage177", "run_stage177_operational_safety_rollback_governance.py", "run_stage177_release_gate.py"])
