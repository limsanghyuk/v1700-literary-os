...
def run_stage178_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]
    stage = run_stage178_page06_release_seal(root)
    checks = {
        "previous_stage_gate_pass": _check(_report_pass(root, "release/current/stage177_release_gate_report.json")),
        "stage178_page06_release_seal_pass": _check(stage.get("status") == "pass"),
        "preflight_execution_report_pass": _check(_report_pass(root, "release/current/stage178_preflight_execution_report.json")),
        "gitnexus_preflight_analysis_pass": _check(_gitnexus_analysis_ok(root)),
        "package_comparison_report_pass": _check(_report_pass(root, "release/current/stage178_package_comparison_report.json")),
    ...
    result = {"stage": "178", "baseline_stage": "177", "title": "Page06 Release Seal", "status": "pass" if not issues else "blocked", "issues": issues, "checks": checks, "default_authority_decision": "DENY", **CORE_INVARIANTS, "branchpoint_lineage_preserved": not issues}
    out = root / "release/current/stage178_release_gate_report.json"
...
def _gitnexus_analysis_ok(root: Path) -> bool:
    path = root / "release/current/stage178_gitnexus_preflight_analysis_report.json"
    if not path.exists(): return False
    try: data=json.loads(path.read_text(encoding="utf-8"))
    except Exception: return False
...
def _docs_manifest_ok(root: Path) -> bool:
    expected=["docs/stages/stage178.md", "docs/proposals/stage178_page06_release_seal_proposal.md", "docs/architecture/stage178_page06_release_seal_blueprint.md", "docs/development/stage178_developer_handoff.md", "manifests/stage178_manifest.json", "manifests/stage178_page06_release_seal_manifest.json", "manifests/stage178_branchpoint_trace_manifest.json", "manifests/live_core_stage178_overlay.json", "release/current/stage178_release_asset_manifest.json", "release/current/stage178_page06_release_seal_report.json", "release/current/stage178_preflight_execution_report.json", "release/current/stage178_gitnexus_preflight_analysis_report.json", "release/current/stage178_package_comparison_report.json"]
...
def _procedure_alignment_ok(root: Path) -> bool:
    ...
    return all(token in text for token in ["stage178", "run_stage178_page06_release_seal.py", "run_stage178_release_gate.py"])
