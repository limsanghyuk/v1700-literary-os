...
def run_stage125_release_gate(root: Path | None = None) -> dict:
    root = root or Path(__file__).resolve().parents[3]
    key = str(root.resolve())
    if key in _CACHE:
        return _CACHE[key]
    stage = run_stage125(root)
    policy = stage.get("absorption_policy", {})
    summary = stage.get("summary", {})
    decision = stage.get("governor_decision", {})
    checks = {
        "stage124_baseline_preserved": _check(stage.get("baseline_stage") == "124"),
        "gate25_primary_authority_preserved": _check(policy.get("gate25_primary_authority_preserved") is True and summary.get("primary_gate") == "Gate25"),
        "gate28_secondary_quality_preserved": _check(policy.get("stage123_gate28_secondary_quality_gate_preserved") is True and "Gate28" in summary.get("secondary_gates", [])),
        "gate29_secondary_predictive_preserved": _check(policy.get("stage124_gate29_secondary_predictive_gate_preserved") is True and "Gate29" in summary.get("secondary_gates", [])),
        "governor_decision_pass": _check(decision.get("status") == "pass" and stage.get("status") == "pass"),
    ...
    result = {
        "stage": "125",
        "baseline_stage": "124",
        "title": "Gate25/28/29 Governor Release Gate",
        "status": "pass" if not issues else "blocked",
        "issues": issues,
        "checks": checks,
        "stage125": _compact(stage),
    ...
    out = root / "release/current/stage125_release_gate_report.json"
...
def _docs_manifest_ok(root: Path) -> bool:
    return all((root / rel).exists() for rel in [
        "docs/stages/stage125.md",
        "manifests/stage125_manifest.json",
        "manifests/stage125_gate_governor_manifest.json",
        "release/current/stage125_gate25_28_29_governor_report.json",
        "release/current/stage125_gate_authority_map.json",
        "release/current/stage125_governor_decision.json",
    ])


def _repo_doctor_ready(root: Path) -> bool:
    text = (root / "tools/run_stage72_repo_doctor.py").read_text(encoding="utf-8", errors="ignore")
    return "stage125" in text and "stage125_release_gate" in text and "stage125_gate25_28_29_governor" in text
...
def _clean_packaging_status(root: Path) -> str:
    ...
    canonical = manifest.get("canonical_package") or "V1700_stage125_gate25_28_29_governor_integrated_repository.zip"
    override = os.environ.get("V1700_STAGE125_PACKAGE")
