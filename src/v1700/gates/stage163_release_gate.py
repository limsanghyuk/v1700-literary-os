...
from v1700.stage163 import run_stage163

_CACHE: dict[str, dict[str, Any]] = {}


def run_stage163_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]
    key = str(root.resolve())
    if key in _CACHE:
        return _CACHE[key]
    if _active_version(root) != "stage163":
        existing = _load_report(root, "stage163_release_gate_report.json")
        if existing is not None and existing.get("status") == "pass":
            _CACHE[key] = existing
            return existing

    baseline = _baseline_gate(root)
    stage = run_stage163(root)
    parts = stage.get("parts", {})
    nodes = parts.get("render_plan_nodes", {})
    edges = parts.get("render_plan_edges", {})
    order = parts.get("render_order", {})
    integrity = parts.get("render_plan_integrity", {})
    checksum = parts.get("deterministic_render_plan_checksum", {})
    projection = parts.get("node2_render_plan_projection_matrix", {})
    policy = parts.get("render_plan_policy", {})
    regression = parts.get("regression_snapshot", {})

    checks = {
        "stage162_baseline_gate_pass": _check(baseline.get("status") == "pass"),
        "stage163_report_pass": _check(stage.get("status") == "pass"),
        "deterministic_render_plan_mode_pass": _check(stage.get("mode") == "DETERMINISTIC_RENDER_PLAN_BUILDER_LOCAL_ONLY"),
        "render_plan_nodes_pass": _check(nodes.get("status") == "pass" and nodes.get("node_count", 0) >= 6),
        "render_plan_edges_pass": _check(edges.get("status") == "pass" and edges.get("edge_count", 0) >= 5),
        "render_order_pass": _check(order.get("status") == "pass" and order.get("order_count", 0) == stage.get("node_count")),
        "render_plan_integrity_pass": _check(integrity.get("status") == "pass"),
        "deterministic_checksum_pass": _check(checksum.get("status") == "pass" and len(str(stage.get("render_plan_checksum", ""))) == 64),
        "node2_render_plan_projection_pass": _check(projection.get("status") == "pass" and stage.get("node2_raw_reveal_access") == 0),
        "render_plan_policy_pass": _check(policy.get("status") == "pass" and stage.get("render_plan_write_enabled") is False),
        ...
        "write_operations_blocked": _check(stage.get("render_write_enabled") is False and stage.get("render_plan_write_enabled") is False and stage.get("write_operation_count") == 0),
    ...
    out = root / "release/current/stage163_release_gate_report.json"
...
def _baseline_gate(root: Path) -> dict[str, Any]:
    if _active_version(root) != "stage163":
        report = _load_report(root, "stage162_release_gate_report.json")
        if report is not None and report.get("status") == "pass":
            return report
    return run_stage162_release_gate(root)
...
def _procedure_alignment_ok(root: Path) -> bool:
    ...
    return all(token in contents for token in ["stage163", "run_stage163_deterministic_render_plan_builder.py", "run_stage163_release_gate.py"])
