...
def run_stage180_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]
    if _active_version(root) != "stage180":
        existing = root / "release/current/stage180_release_gate_report.json"
        if existing.exists():
            try: return json.loads(existing.read_text(encoding="utf-8"))
            except Exception: pass
    stage = run_stage180_architecture_drift_self_audit(root)
    checks = {
        "previous_stage_gate_pass": _check(_report_pass(root, "release/current/stage179_release_gate_report.json")),
        "stage180_architecture_drift_self_audit_pass": _check(stage.get("status") == "pass"),
        "preflight_execution_report_pass": _check(_report_pass(root, "release/current/stage180_preflight_execution_report.json")),
        "gitnexus_preflight_analysis_pass": _check(_gitnexus_analysis_ok(root)),
        "package_comparison_report_pass": _check(_report_pass(root, "release/current/stage180_package_comparison_report.json")),
    ...
    result={"stage":"180","baseline_stage":"179","title":"Architecture Drift and Long-Horizon Self-Audit","status":"pass" if not issues else "blocked","issues":issues,"checks":checks,"default_evolution_decision":"DEFER", **CORE_INVARIANTS, "branchpoint_lineage_preserved": not issues}
    out=root/"release/current/stage180_release_gate_report.json"; out.parent.mkdir(parents=True, exist_ok=True); out.write_text(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True)+"\n", encoding="utf-8")
    return result
...
def _gitnexus_analysis_ok(root: Path) -> bool:
    path=root/"release/current/stage180_gitnexus_preflight_analysis_report.json"
    if not path.exists(): return False
    try: data=json.loads(path.read_text(encoding="utf-8"))
    except Exception: return False
...
def _docs_manifest_ok(root: Path) -> bool:
    expected=["docs/stages/stage180.md","docs/proposals/stage180_architecture_drift_self_audit_proposal.md","docs/architecture/stage180_architecture_drift_self_audit_blueprint.md","docs/development/stage180_developer_handoff.md","manifests/stage180_manifest.json","manifests/stage180_architecture_drift_self_audit_manifest.json","manifests/stage180_branchpoint_trace_manifest.json","manifests/live_core_stage180_overlay.json","release/current/stage180_release_asset_manifest.json","release/current/stage180_architecture_drift_self_audit_report.json","release/current/stage180_release_gate_report.json","release/current/stage180_preflight_execution_report.json","release/current/stage180_gitnexus_preflight_analysis_report.json","release/current/stage180_package_comparison_report.json"]
    return all((root/rel).exists() for rel in expected)
def _procedure_alignment_ok(root: Path) -> bool:
    text="\n".join((root/p).read_text(encoding="utf-8") for p in ["README.md","RELEASE_NOTES.md","package_manifest.json","docs/stages/stage180.md","docs/development/stage180_developer_handoff.md"] if (root/p).exists())
    return all(token in text for token in ["stage180", "run_stage180_architecture_drift_self_audit.py", "run_stage180_release_gate.py"])
