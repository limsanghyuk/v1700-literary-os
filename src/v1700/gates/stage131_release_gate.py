...
from v1700.stage131 import run_stage131

_CACHE: dict[str, dict[str, Any]] = {}


def run_stage131_release_gate(root: Path | None = None) -> dict[str, Any]:
    root = root or Path(__file__).resolve().parents[3]
    key = str(root.resolve())
    if key in _CACHE:
        return _CACHE[key]
    baseline = run_stage130_release_gate(root)
    stage = run_stage131(root)
    parts = stage.get("parts", {})
    preflight = parts.get("gitnexus_preflight", {})
    checks = {
        "stage130_baseline_gate_pass": _check(baseline.get("status") == "pass"),
        "classifier_pass": _check(parts.get("classifier", {}).get("status") == "pass"),
        "advisory_policy_pass": _check(parts.get("policy", {}).get("status") == "pass"),
        "gitnexus_python_fallback_preflight_pass": _check(preflight.get("python_fallback", {}).get("status") == "PASS"),
        "symbol_to_branchpoint_trace_pass": _check(preflight.get("release_gate_integration", {}).get("stage131_gate_registered") is True),
        "gate26_advisory_only_pass": _check(stage.get("gate26_hard_block_enabled") is False and stage.get("gate26_hard_block_count") == 0),
        "writer_approval_guard_pass": _check(stage.get("writer_approval_guard") is True and stage.get("true_contradiction_review_required") is True),
    ...
    result = {
        "stage": "131",
        "baseline_stage": "130",
        "title": "GIG / Gate26 Advisory Absorption Gate",
        "status": "pass" if not issues and stage.get("status") == "pass" else "blocked",
        "issues": issues,
        "checks": checks,
        "stage131": _compact(stage),
    ...
    out = root / "release/current/stage131_release_gate_report.json"
...
def _compact(stage: dict[str, Any]) -> dict[str, Any]:
    keep = (
        "status", "stage", "baseline_stage", "title", "issues", "advisory_mode",
...
def _docs_manifest_ok(root: Path) -> bool:
    return all((root / rel).exists() for rel in [
        ...
        "release/current/stage131_gig_advisory_pack/classifier_report.json",
        "release/current/stage131_gig_advisory_pack/policy_report.json",
        "release/current/stage131_gig_advisory_pack/gitnexus_preflight_report.json",
    ])


def _repo_doctor_ready(root: Path) -> bool:
    text = (root / "tools/run_stage72_repo_doctor.py").read_text(encoding="utf-8", errors="ignore")
    return "stage131" in text and "stage131_release_gate" in text and "stage131_gig_advisory" in text
...
def _clean_packaging_status(root: Path) -> str:
    ...
    canonical = manifest.get("canonical_package") or "V1700_stage131_gig_gate26_advisory_absorption_integrated_repository.zip"
    override = os.environ.get("V1700_STAGE131_PACKAGE")
