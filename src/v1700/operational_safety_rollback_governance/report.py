...
TARGET_STAGE = "stage177"
TARGET_REPORT = "release/current/stage177_operational_safety_rollback_governance_report.json"
PACK_DIR = "release/current/stage177_operational_safety_rollback_governance_pack"
...
def run_stage177_operational_safety_rollback_governance(root: Path | None = None, mode: str = "active") -> dict[str, Any]:
    ...
    previous_gate = _load_json(root / "release/current/stage176_release_gate_report.json") or {}
    previous_report = _load_json(root / "release/current/stage176_lineage_review_gate_report.json") or {}
    preflight = _load_json(root / "release/current/stage177_preflight_execution_report.json") or {}
    gitnexus = _load_json(root / "release/current/stage177_gitnexus_preflight_analysis_report.json") or {}
    comparison = _load_json(root / "release/current/stage177_package_comparison_report.json") or {}

    parts = _build_stage177_pack(root, previous_gate, previous_report, preflight, gitnexus, comparison)
    ...
    result: dict[str, Any] = {
        "stage": "177", "baseline_stage": "176", "title": "Operational Safety and Rollback Governance",
        "status": "pass" if not issues else "blocked", "issues": issues,
        "page": "Page06 Governance Body", "mode": "OPERATIONAL_SAFETY_AND_ROLLBACK_GOVERNANCE_LOCAL_ONLY",
        ...
        "stage178_page06_release_seal_ready": not issues,
        "next_stage": "stage178", "next_stage_title": "Page06 Release Seal",
        **CORE_INVARIANTS,
        "default_authority_decision": "DENY", "deny_by_default": True,
        "branchpoint_lineage_preserved": not issues,
        "parts": {"previous_gate": _compact(previous_gate), **parts},
    }
    result["stage177_checksum"] = _stable_digest(result)
    _write_json(root / TARGET_REPORT, result)
    _write_json(root / "release/current/stage177_summary.json", _compact(result))
    return result


def _build_stage177_pack(root: Path, previous_gate: dict[str, Any], previous_report: dict[str, Any], preflight: dict[str, Any], gitnexus: dict[str, Any], comparison: dict[str, Any]) -> dict[str, dict[str, Any]]:
    stage = TARGET_STAGE
    if stage == "stage174":
...
def _blocked(issue: str) -> dict[str, Any]:
    return {"stage": "177", "baseline_stage": "176", "title": "Operational Safety and Rollback Governance", "status": "blocked", "issues": [issue], "page": "Page06 Governance Body", **CORE_INVARIANTS, "branchpoint_lineage_preserved": False}
