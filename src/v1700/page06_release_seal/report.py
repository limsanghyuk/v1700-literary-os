...
TARGET_STAGE = "stage178"
TARGET_REPORT = "release/current/stage178_page06_release_seal_report.json"
PACK_DIR = "release/current/stage178_page06_release_seal_pack"
...
def run_stage178_page06_release_seal(root: Path | None = None, mode: str = "active") -> dict[str, Any]:
    ...
    previous_gate = _load_json(root / "release/current/stage177_release_gate_report.json") or {}
    previous_report = _load_json(root / "release/current/stage177_operational_safety_rollback_governance_report.json") or {}
    preflight = _load_json(root / "release/current/stage178_preflight_execution_report.json") or {}
    gitnexus = _load_json(root / "release/current/stage178_gitnexus_preflight_analysis_report.json") or {}
    comparison = _load_json(root / "release/current/stage178_package_comparison_report.json") or {}

    parts = _build_page06_release_seal_pack(root, previous_gate, previous_report, preflight, gitnexus, comparison)
    ...
    result: dict[str, Any] = {
        "stage": "178", "baseline_stage": "177", "title": "Page06 Release Seal",
        "status": "pass" if not issues else "blocked", "issues": issues,
        "page": "Page06 Governance Body", "mode": "PAGE06_RELEASE_SEAL_LOCAL_ONLY",
        ...
        "stage179_evolution_body_ready": not issues,
        "next_stage": "stage179", "next_stage_title": "Evolution Body",
        **CORE_INVARIANTS,
        "default_authority_decision": "DENY", "deny_by_default": True,
        "branchpoint_lineage_preserved": not issues,
        "parts": {"previous_gate": _compact(previous_gate), **parts},
    }
    result["stage178_checksum"] = _stable_digest(result)
    _write_json(root / TARGET_REPORT, result)
    _write_json(root / "release/current/stage178_summary.json", _compact(result))
    return result


def _build_stage178_pack(root: Path, previous_gate: dict[str, Any], previous_report: dict[str, Any], preflight: dict[str, Any], gitnexus: dict[str, Any], comparison: dict[str, Any]) -> dict[str, dict[str, Any]]:
    stage = TARGET_STAGE
    if stage == "stage174":
...
def _blocked(issue: str) -> dict[str, Any]:
    return {"stage": "178", "baseline_stage": "177", "title": "Page06 Release Seal", "status": "blocked", "issues": [issue], "page": "Page06 Governance Body", **CORE_INVARIANTS, "branchpoint_lineage_preserved": False}
