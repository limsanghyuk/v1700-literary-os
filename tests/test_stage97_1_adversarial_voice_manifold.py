...
def test_blocks_unexplained_style_drift():
    report = run_stage97_1_adversarial_validation(ROOT)
    result = next(item for item in report["results"] if item["case_id"] == "ADV-VOC-001")
    assert result["actual_status"] == "BLOCK"
    assert result["triggered_gate"] == "voice_manifold"
