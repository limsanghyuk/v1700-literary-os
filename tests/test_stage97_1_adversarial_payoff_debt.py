...
def test_blocks_defaulted_critical_payoff_debt():
    report = run_stage97_1_adversarial_validation(ROOT)
    result = next(item for item in report["results"] if item["case_id"] == "ADV-PAY-001")
    assert result["actual_status"] == "BLOCK"
    assert result["triggered_gate"] == "payoff_debt_ledger"
