...
from v1700.gates.stage174_release_gate import run_stage174_release_gate

ROOT = Path(__file__).resolve().parents[1]

def test_stage174_release_policy_registry_passes() -> None:
    result = run_stage174_release_policy_registry(ROOT, mode="historical")
...
def test_stage174_release_gate_requires_gitnexus_7x12() -> None:
    result = run_stage174_release_gate(ROOT)
    assert result["status"] == "pass"
...
def test_stage174_active_version_mismatch_blocks(tmp_path: Path) -> None:
    sandbox = tmp_path / "repo"
    _copy_minimal_repo(ROOT, sandbox)
    manifest_path = sandbox / "manifests/live_core_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["active_version"] = "stage173"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2)+"\n", encoding="utf-8")
    result = run_stage174_release_policy_registry(sandbox)
