...
from v1700.gates.stage182_release_gate import run_stage182_release_gate
ROOT=Path(__file__).resolve().parents[1]
def test_stage182_upgrade_simulation_compatibility_sandbox_passes() -> None:
    result=run_stage182_upgrade_simulation_compatibility_sandbox(ROOT, mode="historical")
...
def test_stage182_release_gate_requires_gitnexus_7x12() -> None:
    result=run_stage182_release_gate(ROOT)
    assert result["status"] == "pass"
